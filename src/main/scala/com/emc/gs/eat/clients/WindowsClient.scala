...
class WindowsClient(wmiClientLocation: String, outputDir: String) {
  ...
  def runWindowsGrab(host: Host, index: Int): AnalyzeHostResult = {
    ...
    new AnalyzeHostResult(host, "%s grab processed successfully using windows client".format(host.address))
  }
